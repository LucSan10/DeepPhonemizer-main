...
def evaluate_samples(lang_samples: Dict[str, List[Tuple[List[str], List[str], List[str]]]]) -> Dict[str, Any]:
    ...
    for lang in languages:
        for word, generated, target in lang_samples[lang]:
            ...
            trans_edd, trans_count = phoneme_edit_distance(generated, target)
            trans_err = phoneme_error(generated, target)
            
            syll_edd, syll_count = syllable_edit_distance(generated, target)
            syll_err = syllable_error(generated, target)
            syllsize_err = syllable_size_error(generated, target)
            ...
            syllsize_err_dict = lang_syllsize_err.setdefault(lang, dict())
            ...
            if best_trans_err is None or trans_edd / trans_count < best_trans_err / best_trans_count:
                ...
                phon_edd_dict[word] = trans_edd
                phon_count_dict[word] = trans_count
                phon_err_dict[word] = trans_err
                
                syll_edd_dict[word] = syll_edd
                syll_count_dict[word] = syll_count
                syll_err_dict[word] = syll_err
                
                syllsize_err_dict[word] = syllsize_err
    ...
    for lang in languages:
        ...
        syllsize_err = sum(lang_syllsize_err[lang].values())
        syllsize_errors.append(syllsize_err)
        ...
        syllsize_error_rate = syllsize_err / syll_count
        ...
        evaluation_result.setdefault(lang, {}).update({'syll_size_error_rate': syllsize_error_rate})
    ...
    mean_syllsize_error_rate = sum(syllsize_errors) / sum(syll_counts)
    ...
    evaluation_result['mean_syllsize_error_rate'] = mean_syllsize_error_rate
